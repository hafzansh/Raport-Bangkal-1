...
{
    public partial class mp_pjok : Page
    {
        public mp_pjok()
        {

            InitializeComponent();
            Constants.current1 = Database.kd_pjok3;
            Constants.current3 = Database.kd_pjok4;
            Constants.current2 = Database.kkm_pjok;            
            validate();
            try
            {
                clearTable();
        ...
        private void fillConnect()
        {
            ...
            data_kd.ItemsSource = Connection.dataset.Tables["kd_pjok3"].DefaultView;
            data.ItemsSource = Connection.dataset.Tables[Constants.pjok_title].DefaultView;
            data_kd2.ItemsSource = Connection.dataset.Tables["kd_pjok4"].DefaultView;
            data2.ItemsSource = Connection.dataset.Tables[Constants.pjok_title2].DefaultView;
        }
        private void clearTable()
        {
            Connection.dataset.Tables[Constants.pjok_title].Clear();
            Connection.dataset.Tables[Constants.pjok_title2].Clear();
            Connection.dataset.Tables["kd_pjok3"].Clear();
            Connection.dataset.Tables["kd_pjok4"].Clear();
        }
        private void validate()
        {
            ExpanderKD3.Title = "Aspek Pengetahuan (" + Constants.current1 + ")";
            ExpanderKD4.Title = "Aspek Keterampilan (" + Constants.current3 + ")";
            if (Database.kd_pjok3 == 1)
            {
