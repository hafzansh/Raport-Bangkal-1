...
{
    public partial class mp_bta : Page
    {
        public mp_bta()
        {

            InitializeComponent();
            Constants.current1 = Database.kd_bta3;
            Constants.current3 = Database.kd_bta4;
            Constants.current2 = Database.kkm_bta;            
            validate();
            try
            {
                clearTable();
        ...
        private void fillConnect()
        {
            ...
            data_kd.ItemsSource = Connection.dataset.Tables["kd_bta3"].DefaultView;
            data.ItemsSource = Connection.dataset.Tables[Constants.bta_title].DefaultView;
            data_kd2.ItemsSource = Connection.dataset.Tables["kd_bta4"].DefaultView;
            data2.ItemsSource = Connection.dataset.Tables[Constants.bta_title2].DefaultView;
        }
        private void clearTable()
        {
            Connection.dataset.Tables[Constants.bta_title].Clear();
            Connection.dataset.Tables[Constants.bta_title2].Clear();
            Connection.dataset.Tables["kd_bta3"].Clear();
            Connection.dataset.Tables["kd_bta4"].Clear();
        }
        private void validate()
        {
            ExpanderKD3.Title = "Aspek Pengetahuan (" + Constants.current1 + ")";
            ExpanderKD4.Title = "Aspek Keterampilan (" + Constants.current3 + ")";
            if (Database.kd_bta3 == 1)
            {
